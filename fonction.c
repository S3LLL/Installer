...
void *lancerThreadUsb(void *args){      /* fonction qui lance le script d'installation USB  */
    ...
    retour = lancerScriptInt("/var/lib/installFrogLinux/instal_disque_cle.sh",NULL);
...
void *lancerThreadDur(void *args){      /* fonction qui lance le script d'installation Disque rigide auto */
    ...
    retour= lancerScriptInt("/var/lib/installFrogLinux/install_disque_usb.sh",NULL);  //NULL = pas de parametre pour le script donc compris automatique pour lui
